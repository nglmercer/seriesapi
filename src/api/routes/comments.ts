...
export async function handleCommentPost(req: Request, db: Database): Promise<Response> {
  try {
    ...
    try {
      ...
      return badRequest("Invalid JSON body.", locale);
    }
    ...
    if (!Number.isInteger(entity_id) || entity_id < 1) {
      return badRequest("entity_id must be a positive integer.", locale);
    }
    if (
      typeof display_name !== "string" ||
      display_name.trim().length < 2 ||
      display_name.trim().length > MAX_DISPLAY_NAME
    ) {
      return badRequest(
        `display_name must be between 2 and ${MAX_DISPLAY_NAME} characters.`,
        locale,
      );
    }
    if (
      typeof text !== "string" ||
      text.trim().length < 1 ||
      text.trim().length > MAX_BODY
    ) {
      return badRequest(`body must be between 1 and ${MAX_BODY} characters.`, locale);
    }
    ...
    if (parent_id !== null) {
      const parent = db
        .query(
          "SELECT id FROM comments WHERE id = ? AND entity_type = ? AND entity_id = ?",
        )
        .get([parent_id, entity_type, entity_id]);
      if (!parent) return badRequest("parent_id does not exist or is in a different thread.", locale);
    }
    ...
    const result = db.run(
      `INSERT INTO comments
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [entity_type, entity_id, parent_id, display_name.trim(), ip_hash,
       commentLocale, text.trim(), contains_spoilers],
    );
    ...
    return ok(created, { locale }, 201);
  } catch (err) {
    return serverError(err, "en");
  }
}

export function handleCommentGet(req: Request, db: Database, id: number): Response {
  try {
    const locale = getLocaleFromRequest(req, SUPPORTED_LOCALES);
